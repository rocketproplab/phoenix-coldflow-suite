...
const uint8_t ARMED = gn2_lng_flow_mask | gn2_lox_flow_mask;   // Armed: ready to launch, waiting trigger
const uint8_t LAUNCH = gn2_lng_flow_mask | gn2_lox_flow_mask | lng_flow_mask | lox_flow_mask;  // Launch: ignition/flight started
...
void receiveSensorData()
{
  ...
  if (payloadLen >= sizeof(csv)) return;       // corrupted – drop
  memcpy(csv, buffer + 14, payloadLen);
  csv[payloadLen] = '\0';
  ...
  while (tok && idx < 10) {
    float v = atof(tok);
    if      (idx < 5) pt[idx]       = v;
    else if (idx < 8) lc[idx - 5]   = v;
    else              tc[idx - 8]   = v;
    tok = strtok(nullptr, ",");
    ++idx;
  }
  if (idx != 10) return;                       // malformed – drop
